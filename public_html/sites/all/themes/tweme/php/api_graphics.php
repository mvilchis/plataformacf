...
curl_setopt($ch, CURLOPT_URL,"http://api.datos.gob.mx/v1/cf.metadata?pageSize=99999");
$result=curl_exec($ch);
curl_close($ch);
...
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_URL,"http://api.datos.gob.mx/v1/cf.geo?pageSize=999999");
$result=curl_exec($ch);
curl_close($ch);
...
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_URL,"http://api.datos.gob.mx/v1/cf.grupos?pageSize=999999");
$result=curl_exec($ch);
curl_close($ch);
...
<script type="text/javascript">
	...
	function exportToPDF(i) {
		(function ($) {
			if (i == "map") {11 svg = $("svg.leaflet-zoom-animated")[0];
			}
			else if (i == "line") {
				(function ($) {
	...
	function change_variable() {
		...
		(function ($) {
			...
			$.getJSON("http://api.datos.gob.mx/v1/cf.datos", params, function (data) {		
				<?php if ($page == "compara"): ?>
				$.getJSON("http://api.datos.gob.mx/v1/cf.datos", { id: $("select#select-indicador-b option:selected").val(), pageSize: 999999, id2: 'a'}, function (data_b) {
				<?php endif; ?>
