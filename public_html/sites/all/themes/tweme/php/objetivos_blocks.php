...
curl_setopt($ch, CURLOPT_URL,"http://api.datos.gob.mx/v1/cf.metadata");
$result=curl_exec($ch);
curl_close($ch);
	...
	function show_objetivo(nom,num) {
		(function ($) {
			...
			$.each(indicadores, function (obj,ind) {
				$.getJSON("http://api.datos.gob.mx/v1/cf.datos", { id: ind.Clave, pageSize: 999999, DesGeo: "N" }, function (data) {
					greatest_year = 0;
					selected_key = 0;
